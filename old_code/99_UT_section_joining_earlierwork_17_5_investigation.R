...
#aims to capture all section 250 with ranges starting with "17"
temp_geocheck <- firstdivisions_geo %>% 
  filter(str_sub(PLSSID, 1L, 12L) == "UT260250S017") 

temp_geocheck2 <- temp_geocheck
st_geometry(temp_geocheck2) <- NULL
temp_geocheck2 %>% 
  count(PLSSID)


#map potential areas for the mystery 17.5 range?
tm_basemap(leaflet::providers$CartoDB.Voyager) +
...
#This looks like it could be it! Here are two records, one that was withdrawn, that might give us an answer here (hopefully!)
#again these are OUR generated plssids, so the first one is the mystery one that broke the pattern
phills_all %>% 
  filter(PLSSID %in% c("UT260250S17.500", "UT260250S0172E0"),
         FRSTDIVNO %in% c("00", "01")) %>% 
  View()
